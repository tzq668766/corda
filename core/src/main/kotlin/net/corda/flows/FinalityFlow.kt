 ...
 */
class FinalityFlow(val transactions: Set<SignedTransaction>,
                   val extraRecipients: Set<Party>,
                   override val progressTracker: ProgressTracker) : FlowLogic<Unit>() {
    constructor(transaction: SignedTransaction, extraParticipants: Set<Party>) : this(setOf(transaction), extraParticipants, tracker())
    constructor(transaction: SignedTransaction) : this(setOf(transaction), emptySet(), tracker())
    ...
    override fun call() {
        ...
        // Send to the notary if needed, record locally and distribute.
        progressTracker.currentStep = NOTARISING
        val notarisedTxns = notariseAndRecord(lookupParties(resolveWithin(transactions)))

        // Each transaction has its own set of recipients, but extra recipients get them all.
        progressTracker.currentStep = BROADCASTING
        for ((stx, parties) in notarisedTxns) {
            subFlow(BroadcastTransactionFlow(stx, parties + extraRecipients))
        }
    }
    ...
    private fun resolveWithin(signedTransactions: Set<SignedTransaction>): List<Pair<SignedTransaction, LedgerTransaction>> {
