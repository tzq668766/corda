...
class TraderDemoClientApi(val rpc: CordaRPCOps) {
    ...
    fun runBuyer(amount: Amount<Currency> = 30000.0.DOLLARS): Boolean {
        ...
        val resultFutures = amounts.map {
            rpc.startFlow(::IssuanceRequester, amount, me.legalIdentity, OpaqueBytes.of(1), bankOfCordaParty).returnValue.toFuture()
        }
    ...
    fun runSeller(amount: Amount<Currency> = 1000.0.DOLLARS, counterparty: String): Boolean {
        ...
        if (otherParty != null) {
            ...
            val stx = rpc.startFlow(::SellerFlow, otherParty, amount).returnValue.toFuture().getOrThrow()
