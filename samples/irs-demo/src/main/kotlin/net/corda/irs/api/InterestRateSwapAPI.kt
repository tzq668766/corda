...
class InterestRateSwapAPI(val rpc: CordaRPCOps) {
    ...
    fun storeDeal(newDeal: InterestRateSwap.State): Response {
        return try {
            rpc.startFlow(AutoOfferFlow::Requester, newDeal).returnValue.toFuture().getOrThrow()
    ...
    fun storeDemoDate(newDemoDate: LocalDate): Response {
        ...
        if (newDemoDate.isAfter(priorDemoDate)) {
            rpc.startFlow(UpdateBusinessDayFlow::Broadcast, newDemoDate).returnValue.toFuture().getOrThrow()
    ...
    fun exitServer(): Response {
        rpc.startFlow(ExitServerFlow::Broadcast, 83).returnValue.toFuture().getOrThrow()
        return Response.ok().build()
    }
}
